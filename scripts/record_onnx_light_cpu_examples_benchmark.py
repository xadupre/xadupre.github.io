...
#: ``producer_name`` stamped on models this repository generates. Only these
#: models are eligible for IR-version clamping; models produced elsewhere are
#: left untouched so any incompatibility surfaces as an error.
_PRODUCER_NAME = "xadupre.github.io"
...
def _make_onnxruntime_session(serialized: bytes):
    ...
    an IR version ``onnxruntime`` refuses to load. When that happens, and only
    for models generated by this repository (identified by ``producer_name``),
    the model's ``ir_version`` is lowered to the maximum ``onnxruntime`` reports
    as supported and the session is retried, so the benchmark keeps working
    across mismatched ``onnx`` / ``onnxruntime`` releases. Models produced
    elsewhere are never modified and the error is raised.
    """
    import onnxruntime
    ...
    try:
        ...
        if model.producer_name != _PRODUCER_NAME:
            raise
