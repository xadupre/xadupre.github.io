...
class TestRecordSizeOnnxLightWorkflow(unittest.TestCase):
    ...
    def test_schema_comparison_reclassifies_expanded_backend_tests(self):
        ...
        # onnx-light uses the same predicate as onnx (only ``_expanded`` suffix)
        self.assertNotIn("def _is_expanded_onnx_light_test_name(name):", content)
        self.assertNotIn('name.startswith("test_cc_")', content)
