...
class TestRecordSizeOnnxLightWorkflow(unittest.TestCase):
    ...
    def test_schema_comparison_reclassifies_expanded_backend_tests(self):
        ...
        # onnx-light uses a dedicated predicate that also covers ``test_cc_*``
        self.assertIn("def _is_expanded_onnx_light_test_name(name):", content)
        self.assertIn('name.startswith("test_cc_")', content)
        self.assertIn("if not _is_expanded_onnx_light_test_name(name):", content)
