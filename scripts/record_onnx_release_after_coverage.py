"""Record how well onnx-light's release-after metadata algorithm matches the
backend tests tagged ``release_after`` by default.
...
``dashboard/onnx-light/release-after-coverage.html`` consumes that file to
show how many test cases, nodes and metadata annotations are recovered exactly.

Usage::

    python scripts/record_onnx_release_after_coverage.py [--cache-dir DIR]
        [--tag release_after] [--limit N]
"""
...
def _node_metadata(node) -> Dict[str, str]:
    """Return the metadata keys relevant to the release-after analysis."""
    return {
...
def discover_release_after_tests(tag=DEFAULT_TAGS) -> List[Dict[str, Any]]:
    ...
    relevant to the release-after analysis and ``expected_values`` is the per-value
...
def build_payload(
    tag=DEFAULT_TAG,
    *,
    discover: Callable[..., List[Dict[str, Any]]] = discover_release_after_tests,
    run: Callable[[Any], Dict[str, Any]] = run_release_after_analysis,
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    json_path = os.path.join(
        args.cache_dir, "onnx-light", "release_after_coverage.json"
    )
    try:
        payload = build_payload(tag=args.tag, limit=args.limit)
    except Exception as exc:  # noqa: BLE001
        _log(f"ERROR: failed to record release after coverage: {exc}")
        traceback.print_exc()
        return 1
