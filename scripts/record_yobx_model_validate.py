...
# is generated by the protobuf compiler and may be incompatible with the
# ``protobuf`` version pulled in by ``onnx``:
#
#  * protobuf 3.x: supports both C++ and pure-Python descriptor implementations.
#    Setting ``PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python`` switches to the
#    pure-Python one, which is tolerant of older generated files.
#  * protobuf 4.x/5.x/7.x: removed the pure-Python implementation entirely, so
#    the env-var workaround above has no effect.  When ``sentencepiece`` is
#    compiled against protobuf 3.x (i.e. the ``sentencepiece_model_pb2`` file
#    uses old-style ``MakeDescriptor`` calls), importing it raises
#    ``TypeError: Descriptors cannot be created directly``.  ``transformers``
#    catches that TypeError in ``import_protobuf()`` only as an ``ImportError``,
#    so the TypeError propagates and the tokenizer step fails with the
#    misleading ``Couldn't instantiate the backend tokenizer … You need to have
#    sentencepiece or tiktoken installed`` message.
#
# The belt-and-suspenders env var below still helps for legacy protobuf 3.x
# environments.  For protobuf 4.x+ the real fix is applied lazily by
# :func:`_fix_sentencepiece_protobuf_compat` (called before the first tokenizer
# load): it detects that ``sentencepiece_model_pb2`` cannot be imported and
# injects ``transformers``' own protobuf-4.x+ compatible replacement
# (``transformers.utils.sentencepiece_model_pb2_new``) into ``sys.modules`` so
# that ``transformers``' ``import_protobuf()`` finds a working implementation.
os.environ.setdefault("PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION", "python")

_sentencepiece_protobuf_fixed: bool = False


def _fix_sentencepiece_protobuf_compat() -> None:
    """Fix ``sentencepiece_model_pb2`` incompatibility with protobuf >= 4.

    ``sentencepiece`` may ship a ``sentencepiece_model_pb2`` module generated
    with the protobuf-3.x compiler.  With protobuf 4.x/7.x that import raises
    ``TypeError: Descriptors cannot be created directly`` at module
    initialisation time.  ``transformers``' ``import_protobuf()`` tries this
    import first (when sentencepiece is installed) and does not catch
    ``TypeError``, so the exception propagates and the tokenizer step fails
    with the misleading "need sentencepiece or tiktoken" message even though
    both packages are installed.

    ``transformers`` ships its own compatible copy of the protobuf-generated
    code in ``transformers.utils.sentencepiece_model_pb2_new`` (regenerated
    with protobuf 4.x-style ``AddSerializedFile``).  This function detects the
    incompatibility at first call and injects that copy into ``sys.modules`` as
    ``sentencepiece.sentencepiece_model_pb2`` so that ``import_protobuf()``
    finds a working implementation on its next invocation.
    """
    global _sentencepiece_protobuf_fixed
    if _sentencepiece_protobuf_fixed:
        return
    _sentencepiece_protobuf_fixed = True

    try:
        import sentencepiece.sentencepiece_model_pb2  # noqa: F401

        # Import succeeded – no patch required.
        return
    except TypeError:
        # sentencepiece_model_pb2 uses old protobuf 3.x descriptor API which
        # is incompatible with protobuf 4.x+.  Fall through to inject the fix.
        pass
    except (ImportError, AttributeError):
        # sentencepiece is not installed or the submodule does not exist at
        # all.  Nothing we can do here; the tokenizer will fail gracefully.
        return

    # Try to inject transformers' compatible protobuf-4.x replacement.
    try:
        ...
        if _pb_ver >= (4, 0):
            ...
            from transformers.utils import sentencepiece_model_pb2 as _spm_pb2_compat

        import sentencepiece as _sp

        # Expose as both an attribute on the sentencepiece package and as a
        # cached entry in sys.modules so that
        # ``from sentencepiece import sentencepiece_model_pb2`` resolves to
        # the compatible implementation on future invocations.
        setattr(_sp, "sentencepiece_model_pb2", _spm_pb2_compat)
        sys.modules["sentencepiece.sentencepiece_model_pb2"] = _spm_pb2_compat
    except Exception:  # noqa: BLE001 – best-effort, tokenizer will still fail gracefully
        pass
...
def run_validate_one(
    ...
    # Ensure sentencepiece_model_pb2 is compatible with the installed protobuf
    # before the tokenizer is loaded.  See :func:`_fix_sentencepiece_protobuf_compat`.
    _fix_sentencepiece_protobuf_compat()
