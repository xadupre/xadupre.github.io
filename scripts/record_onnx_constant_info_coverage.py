"""Record how well onnx-light's constant-information analysis matches the
backend tests tagged ``constant``.
...
keeps only the cases whose ``tag`` matches ``constant`` by default, reruns the
constant-information analysis on each model, and compares the resulting node and
value metadata against the metadata embedded in the test case.

A *constant* value is one whose content is known before inference starts (graph
initializers, ``Constant`` node outputs and, more generally, any deterministic
node all of whose inputs are constant). onnx-light records the result under the
``onnx_light.constant`` metadata key (value ``"1"``) on constant nodes and on
constant values (graph inputs, outputs, ``value_info`` and initializers).

The resulting JSON summary is persisted to
``cache_data/onnx-light/constant_info_coverage.json``. The dashboard at
``dashboard/onnx-light/constant-info-coverage.html`` consumes that file to
show how many test cases, nodes and metadata annotations are recovered exactly.

Usage::

    python scripts/record_onnx_constant_info_coverage.py [--cache-dir DIR]
        [--tag constant] [--limit N]
"""
...
# ``onnx_light.constant`` is stored (value ``"1"``) on every constant node and
# on every constant value proto (input/output/initializer/value_info).
CONSTANT_METADATA_KEY: str = "onnx_light.constant"
METADATA_KEYS: Tuple[str, ...] = (CONSTANT_METADATA_KEY,)
VALUE_METADATA_KEYS: Tuple[str, ...] = (CONSTANT_METADATA_KEY,)
...
def _node_metadata(node) -> Dict[str, str]:
    """Return the metadata keys relevant to the constant-info analysis."""
    return {
...
def _graph_value_snapshot(model) -> List[Dict[str, Any]]:
    ...
    onnx-light may record constant metadata on ``value_info`` only for graph
    inputs/outputs/initializers; those entries are merged into the matching
    entry so that the snapshot reflects the full set of annotations regardless
    of where onnx-light chose to write them.
    """
    ...
    for vi in graph.value_info:
        ...
        if existing is not None:
            ...
            # that metadata stored only in value_info is visible for those values.
...
def _has_expected_value_metadata(values: Optional[List[Dict[str, Any]]]) -> bool:
    """Tell if any expected graph value carries constant-info metadata."""
...
def discover_constant_info_tests(tag=DEFAULT_TAGS) -> List[Dict[str, Any]]:
    ...
    relevant to the constant-info analysis and ``expected_values`` is the
    per-value metadata (inputs, outputs, initializers).
    """
...
def run_constant_info_analysis(model) -> Dict[str, Any]:
    """Run onnx-light's constant-information metadata analysis on ``model``."""
    ...
    write_constant_info_to_metadata(work.graph)
...
def _score_test(
    ...
    if expected_values is not None or actual_values is not None:
        ...
        for val_name in all_names:
            ...
            exp_tags = str(exp_meta.get(CONSTANT_METADATA_KEY, "")).strip()
            act_tags = str(act_meta.get(CONSTANT_METADATA_KEY, "")).strip()
            # Values that are not constant on either side carry no constant-info
            # signal (non-constant is the default) and should not dilute the ratio.
    ...
    # them, it trivially passes but carries no signal about constant-info quality.
    # In constant-info coverage context this means the expected annotations are
...
def build_payload(
    tag=DEFAULT_TAG,
    *,
    discover: Callable[..., List[Dict[str, Any]]] = discover_constant_info_tests,
    run: Callable[[Any], Dict[str, Any]] = run_constant_info_analysis,
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    json_path = os.path.join(
        args.cache_dir, "onnx-light", "constant_info_coverage.json"
    )
    try:
        payload = build_payload(tag=args.tag, limit=args.limit)
    except Exception as exc:  # noqa: BLE001
        _log(f"ERROR: failed to record constant-info coverage: {exc}")
        traceback.print_exc()
        return 1
