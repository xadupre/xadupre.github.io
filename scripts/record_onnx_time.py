...
CSV_FIELDS = ("date", "commit", "run_id", "name", "avg", "median", "max", "std")
_TIMING = re.compile(
...
def append_snapshot(
    html_path: str, csv_path: str, date: str, commit: str, run_id: str
) -> int:
    ...
    if os.path.exists(csv_path):
        with open(csv_path, newline="", encoding="utf-8") as stream:
            existing = {row["run_id"] for row in csv.DictReader(stream)}
    ...
    with open(csv_path, "a", newline="", encoding="utf-8") as stream:
        ...
        for row in rows:
            ...
            writer.writerow(
                {"date": date, "commit": commit, "run_id": run_id, **row}
            )
    return len(rows)
...
def main() -> None:
    ...
    count = append_snapshot(
        args.html, args.output, args.date, args.commit, args.run_id
    )
