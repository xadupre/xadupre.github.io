...
(``ModelProto``, ``GraphProto``, ``NodeProto``, ``TensorProto``, ...) are
...
``ParseFromString`` on the result), and the version converter.  For large
models this serialize/parse pair dominates the wall-clock time of the
call even when the C++ work itself is cheap.

``onnx_light`` does not have this split: the Python ``ModelProto`` exposed
by ``onnx_light.onnx`` *is* the C++ ``ModelProto``, bound through
