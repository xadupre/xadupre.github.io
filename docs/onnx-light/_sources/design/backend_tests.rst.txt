...
:mod:`onnx_light.onnx_lib.backend.test.case` and mirrors the structure of the
official ONNX backend test suite.  The registered node test cases are
generated by the C++ :epkg:`lib_onnx_backend_test` library and exposed to
Python through :func:`~onnx_light.onnx_lib.backend.test.case.collect_test_case`.
Downstream code can still register additional Python-only test cases
by subclassing :class:`~onnx_light.onnx_lib.backend.test.case.Base` and
calling the :func:`~onnx_light.onnx_lib.backend.test.case.expect` helper.
The :func:`~onnx_light.onnx_lib.backend.test.case.make_test_class` function
...
* Python side: :mod:`onnx_light.onnx_lib.backend.test.case` (test-class generation,
...
Call :func:`~onnx_light.onnx_lib.backend.test.case.make_test_class` with
...
The helper
:func:`~onnx_light.onnx_lib.backend.test.case.get_test_cases_for_op` returns
...
:func:`~onnx_light.onnx_lib.backend.test.case.collect_test_case` internally.
...
The file
:epkg:`unittests/onnxl_vs_ort/test_backend_with_onnxruntime.py`
in the
...
:func:`~onnx_light.onnx_lib.backend.test.case.collect_test_case` first
collects every node test case registered by the C++
:epkg:`lib_onnx_backend_test` library (exposed through the
``onnx_light.onnx_py._onnxpybackend.backend_test`` Python bindings). It then
runs every ``export_*`` class method declared on any user-defined
subclass of :class:`~onnx_light.onnx_lib.backend.test.case.Base`; each call
to :func:`~onnx_light.onnx_lib.backend.test.case.expect` appends one
:class:`~onnx_light.onnx.backend.TestCase` to the global
``ALL_TESTS`` dictionary. Python-defined cases take precedence over
C++ cases with the same name.

:func:`~onnx_light.onnx_lib.backend.test.case.make_test_class` calls
:func:`~onnx_light.onnx_lib.backend.test.case.collect_test_case` internally,
...
the :epkg:`lib_onnx_backend_test` static library, with no dependency on
Python. It publicly links the :epkg:`lib_onnx_kernels` static library,
...
:epkg:`onnx_light/onnx_backend_test/cases`\ ``/<group>/`` (``math``, ``logical``,
``nn``, ``tensor``, …) and the expected outputs are computed with the
reference kernels under
:epkg:`onnx_light/onnx_kernels/kernels`\ ``/<group>/`` so the registry is
...
:epkg:`unittests/cc/onnx_kernels` and
:epkg:`unittests/cc/onnx_backend_test`.
